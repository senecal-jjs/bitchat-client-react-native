...
const toBinaryPayload = (message: Message): Uint8Array | null => {
    ...
    if (message.originalSender) {
        ...
        buffer.append(new Uint8Array([Math.min(encodedOriginalSender.length, 255)]))
    ...
    if (message.recipientNickname != null) {
        const encodedRecipientNickname = textEncoder.encode(message.recipientNickname)
        buffer.append(new Uint8Array([Math.min(encodedRecipientNickname.length, 255)]))
...
const fromBinaryPayload = (data: Uint8Array): Message | null => {
    ...
    const id = textDecoder.decode(dataCopy.slice(offset, offset + idLength)) || crypto.randomUUID()
    offset += idLength
    ...
    const sender = textDecoder.decode(dataCopy.slice(offset, offset + senderLength)) || 'unknown'
    offset += senderLength
    ...
    const contents = textDecoder.decode(dataCopy.slice(offset, offset + contentLength)) || ''
    ...
    if (hasOriginalSender && offset < dataCopy.length) {
        ...
        if (offset + length <= dataCopy.length) {
            originalSender = textDecoder.decode(dataCopy.slice(offset, offset + length)) || null
            offset += length
        }
    }
    ...
    if (hasRecipientNickname && offset < dataCopy.length) {
        const length = dataCopy[offset++]
        if (offset + length <= dataCopy.length) {
            recipientNickname = textDecoder.decode(dataCopy.slice(offset, offset + length)) || null
            offset += length
        }
    }
    ...
    if (hasSenderPeerId && offset < dataCopy.length) {
        const length = dataCopy[offset++]
        if (offset + length <= dataCopy.length) {
            senderPeerId = textDecoder.decode(dataCopy.slice(offset, offset + length)) || null
            offset += length
        }
    }
